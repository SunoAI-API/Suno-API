...
def get_lyrics(lid):
    r = requests.post(f"http://127.0.0.1:8000/lyrics/{lid}", data=json.dumps({"session_id": 1}))
    print(r.text)


def get_info(aid):
    response = requests.post(f"http://127.0.0.1:8000/feed/{aid}", data=json.dumps({"session_id": 1}))
